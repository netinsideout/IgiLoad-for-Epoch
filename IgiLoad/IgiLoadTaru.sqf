    ...
    IL_Taru_Transmission =
    {
        ...
        switch (_args select 0) do {
            ...
            case 'chat_attach': {
                 ...
                 _helico vehicleChat "Pod attached";
            };
            case 'chat_deattach': {
                if (!(player in crew _helico)) exitWith{};
                 _helico vehicleChat "Pod deattached";
            };
            case 'chat_drop_with_parachute': {
                 if (!(player in crew _helico)) exitWith{};
                 _helico vehicleChat "Pod dropped with parachute";
            };
            case 'chat_drop_without_parachute': {
                 if (!(player in crew _helico)) exitWith{};
                 _helico vehicleChat "Pod dropped without parachute";
            };
            case 'sound_attach': {
                 if (!(player in crew _helico)) exitWith{};
...
IL_Client_Control = {
    ...
    else
    {
        ...
        _nearBy = (getPosATL _heli) nearEntities [["Epoch_Male_F","Epoch_Female_F","LandVehicle","Ship","Air"], 300];
        {
            if (isPlayer _x) then {
